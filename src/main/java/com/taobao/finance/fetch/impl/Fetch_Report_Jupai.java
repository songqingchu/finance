...
public class Fetch_Report_Jupai {
	//

	public static String url = "http://guba.eastmoney.com/list,stock,3,f_1.html";

	public static String getUrl(String stock) {
		String newUrl = url.replace("stock", stock);
		return newUrl;
	}

	public static List<Report> fetch(String code, String proxy, Integer port) {
		...
		try {
			...
			if (getMethod.getStatusCode() == 200) {
				...
				l = FetchUtil.parseReport(jsonStr);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String args[]) {
		...
		l.size();
		/*
		 * List<Report> l=Fetch_Report_Jupai.fetch("000848", null, null);
		 * l.size();
		 */
...
class FetchSingle2Thread extends Thread {
	...
	public FetchSingle2Thread(CountDownLatch latch, List<Report> l,
			String code, Integer size, Proxy p) {
		this.latch = latch;
		this.l = l;
		this.code = code;
		this.size = size;
