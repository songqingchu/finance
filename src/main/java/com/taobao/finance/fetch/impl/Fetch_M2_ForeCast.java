...
public class Fetch_M2_ForeCast {
	...
	static {
		...
		String dateDir = dir + "\\";
		map.putAll(FetchUtil.readFileMapAbsolute(dateDir
				+ FetchUtil.FILE_HOLDING_F_ZXB));

		one.addAll(FetchUtil.readFileListAbsolute(dateDir
				+ FetchUtil.FILE_HOLDING_F_ZXB1));

		two.addAll(FetchUtil.readFileListAbsolute(dateDir
				+ FetchUtil.FILE_HOLDING_F_ZXB2));

		three.addAll(FetchUtil.readFileListAbsolute(dateDir
				+ FetchUtil.FILE_HOLDING_F_ZXB3));

		four.addAll(FetchUtil.readFileListAbsolute(dateDir
				+ FetchUtil.FILE_HOLDING_F_ZXB4));
	...
	public static void exam() {
		...
		System.out.println("机选正确率："
				+ FetchUtil.formatRate((float) missing.size()
						/ (float) map.size()));
		System.out.println("机选收益率："
